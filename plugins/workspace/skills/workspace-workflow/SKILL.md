...
description: >-
  The per-issue workspace workflow for a submodule hub — how the hub, worktrees and integration
  branches fit together, the four /workspace commands, filling submodules on demand, and the cmux
  conventions. Read this before running /workspace, /workspace-sync, /update-modules or /teardown,
  or when reasoning about auto-pinning, integration branches, or why a submodule directory is empty.
---

This is the host-neutral core of the per-issue workspace workflow, shared across every **hub** that
uses it. Each hub's own `CLAUDE.md`/`AGENTS.md` adds the repo-specific bits on top (the actual
submodules + remotes, integration branches, tracker/forge conventions). Nothing here names a
specific repository or organization — the engine discovers all of that at runtime.
...
The engine `workspace` (on PATH, from this plugin's `bin/`; shared across hubs and
...
slash commands orchestrate scope confirmation, the hub-repo pin commits, and cmux spawning. The
commands ship with this plugin, so every hub shares one copy.
...
### Shared Claude assets (host-neutral, plugin-managed)

Alongside the commands, this plugin carries two more host-neutral layers, available in every project
wherever the plugin is enabled:

- **`hooks/allow-readonly-traversal.py`** — wired as a `PreToolUse(Bash)` hook in `hooks/hooks.json`;
  it auto-approves provably read-only `cd`+`/dev/null` submodule traversals (and only those — it
  abstains otherwise, keyed to `CLAUDE_PROJECT_DIR`), removing the built-in cd+redirect approval
  prompt.
- **the `cmux` plugin** — the `cmux-*` skill family, shipped separately in the same marketplace
  because cmux is host-neutral and versions on its own cadence. Hubs must **not** keep local copies
  under `<hub>/.claude/skills/`.

To add a hook or skill, edit this plugin and `claude plugin update workspace@alexanderjeurissen`.
...
required semantic palette; rich node content) lives in the **`mermaid-style`** skill, shipped by the
`house-style` plugin in the same marketplace.
